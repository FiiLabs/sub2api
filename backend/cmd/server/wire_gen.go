...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	supplyOverflowCounter := repository.NewSupplyOverflowCounter(client)
	settingService := service.ProvideSettingService(settingRepository, groupRepository, proxyRepository, supplyOverflowCounter, configConfig)
	...
	supplyDemandBalanceService := service.ProvideSupplyDemandBalanceService(settingService, dashboardService, supplierOnboardingRepository)
	authService := service.ProvideAuthService(client, userRepository, redeemCodeRepository, refreshTokenCache, configConfig, settingService, emailService, turnstileService, tencentCaptchaService, aliyunCaptchaService, emailQueueService, promoService, subscriptionService, affiliateService, serviceUserPlatformQuotaRepository, supplyDemandBalanceService)
	...
	manager := payoutchain.ProvideChainManager(settingService, secretEncryptor)
	settingHandler := handler.ProvideAdminSettingHandler(settingService, emailService, turnstileService, aliyunCaptchaService, opsService, paymentConfigService, paymentService, userAttributeService, notificationEmailService, totpService, userService, manager, secretEncryptor, configConfig)
	...
	supplierOnboardingService := service.ProvideSupplierOnboardingService(supplierOnboardingRepository, accountRepository, oAuthService, openAIOAuthService, settingService, supplierIncidentService, usageLogRepository, accountTestService, supplyDemandBalanceService)
	...
	supplierChainClient := payoutchain.ProvideChainClientFromManager(manager)
	...
	supplyMarketHealthRepository := repository.NewSupplyMarketHealthRepository(client)
	supplyMarketHealthService := service.NewSupplyMarketHealthService(supplyMarketHealthRepository, settingService, groupRepository)
	supplierHandler := handler.NewSupplierHandler(supplierOnboardingService, supplierCreditService, supplierAdminService, supplierWithdrawalService, supplierExportService, supplierIncidentService, supplierPayoutWalletService, supplyMarketHealthService)
	homepageStatsService := service.ProvideHomepageStatsService(settingService, dashboardService, supplierOnboardingRepository)
	publicStatsHandler := handler.NewPublicStatsHandler(homepageStatsService)
	publicBannerHandler := handler.NewPublicBannerHandler(settingService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	openAIQuotaAutoResetService := service.ProvideOpenAIQuotaAutoResetService(accountRepository, openAIQuotaService, rateLimitService, idempotencyCoordinator, auditLogService, settingService, leaderLockCache)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, supplierHandler, publicStatsHandler, publicBannerHandler, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
