...
// TestConsultPre_ModelNotInRouteMap: valid key + model NOT in route_map → allow=false, status=404.
func TestConsultPre_ModelNotInRouteMap(t *testing.T) {
	...
	h := newTestConsultHandler(
		&fakeAPIKeys{byHash: map[string]*service.APIKey{"hash-team": validKey}},
		nil,
		&fakePricing{},
		cfg,
	)
...
// TestConsultPre_ValidationDenied: a key whose ValidateAPIKey fails (inactive user) →
...
func TestConsultPre_ValidationDenied(t *testing.T) {
	...
	// Valid key (User non-nil) but user is inactive (Status "disabled", Balance 0).
...
// TestConsultPre_Success: valid key + active user + balance>0 + model in route_map → allow=true.
func TestConsultPre_Success(t *testing.T) {
	...
	h := newTestConsultHandler(
		&fakeAPIKeys{byHash: map[string]*service.APIKey{"hash-ok": validKey}},
		nil,
		&fakePricing{
			data: map[string]*service.LiteLLMModelPricing{
