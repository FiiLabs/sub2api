...
func applySubjectToAPIKey(key *APIKey, subject SubjectResourceContext) {
	key.BillingSubjectID = subject.BillingSubjectID
	key.CreatedByUserID = &subject.ActorUserID
	if subject.TeamID > 0 {
...
type APIKeyListFilters struct {
	...
	CreatedByUserID *int64 // nil=不限制创建者；非 nil=仅该用户创建的 key（团队 self 作用域）
}
