...
const (
	...
	// SupplierFreezeHoursDefault 默认冻结窗（72 小时）。
	//
	// 这个数必须 ≥ 支付通道实际拒付窗，不变量「已释放 = 拒付安全」才成立。
	// 代码侧只 clamp 到上面那个上限，**拦不住「配小了」**——配小了的表现是
	// 冻结期过后钱已经付给供给者，此后再被拒付平台自吃，而这件事要等到
	// 第一笔拒付才看得见。所以改小它必须是一个有记录的决定，而不是顺手。
	//
	// # 2026-08-27 从 720（30 天）改到 72（3 天），理由与代价
	//
	// 这不是"更安全"的选择，是一次明确的权衡：
	//
	//   - 冻结期对供给者是**纯粹的等待成本**，而供给是这个市场的瓶颈侧。
	//     30 天的等待在招募时是硬伤——供给者干完活要等一个月才见到钱。
	//   - 3 天足以覆盖**明显欺诈**（盗卡消费通常在几天内被持卡人发现并拒付），
	//     那是拒付里金额占比最大、也最该被冻结挡住的一类。
	//   - 30 天多覆盖的是**长尾拒付**。用全额冻结去挡长尾，等于让所有供给者
	//     为少数拒付垫资一个月。正确的工具是**比例准备金**（释放大部分、
	//     留一小部分更久），那个还没做——见下面那条明账。
	//
	// **代价写在明处**：3 天到 30 天之间发生的拒付，追不回来了，平台自吃。
	// 这笔损失仍然可观测——它落在 `payment_disputes.uncovered_basis` 上。
	// **上线一个月后按那一列的真实累计值复核**：如果那个数明显超出预期，
	// 要么把这个值调回去，要么把比例准备金做出来。
	//
	// 另有一条能从根上绕开这个权衡的路（也没做）：**加密货币充值产生的消费
	// 根本不需要冻结**——链上支付不可拒付。能追溯到请求的资金来源的话，
	// 那部分收益可以即时释放。
	SupplierFreezeHoursDefault = 72
	// SupplierShareRatioDefault 默认分成比例。
	//
	// 0.80 是定价决策的一部分（与消费者侧倍率联动，两者一起定才有意义）：
	// 分成比例高不等于供给者拿得多，基数是**消费者实付**，倍率压得越低、
	// 同样的比例换来的钱越少。改这个数之前先看运维文档里那份三方分账测算。
	SupplierShareRatioDefault = 0.80
)
