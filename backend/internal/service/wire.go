...
var ProviderSet = wire.NewSet(
	...
	// APEXONE-EXT: 异常使用检测（周期扫描 + 告警）。
	//
	// 2026-09-02 补登记。在此之前它只出现在 wire_gen.go 里——那一行是手写的，
	// ProviderSet 里从来没有过这一条。后果是 `wire ./cmd/server/` 一跑就报
	// "no provider found for *service.AbuseDetectorService"，于是每次同步 upstream
	// 都只能手工合 wire_gen.go，而手工合生成文件正是最容易漏掉一两行的地方。
	ProvideAbuseDetectorService,
