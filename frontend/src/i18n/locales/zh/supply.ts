...
export default {
  supply: {
    ...
    privacy: {
      title: '我们看不到你的聊天记录',
      // 用「钥匙」而不是 scope 名：犹豫这件事是感性的，先把心结解开，
      // 技术细节留给愿意核对的人（下面两行 + 文档链接）。
      body: '你授权给我们的，是一把只能做一件事的钥匙：用你的额度去跑别人的请求。翻看聊天记录是另一把钥匙，我们从头到尾没要过——所以这不是「我们保证不看」，是我们手上根本没有那把钥匙。点同意之前，授权页面会列出我们申请了哪些权限，你可以自己对一遍。',
      scopeGranted: '能做的：用你的额度跑别人的请求',
      scopeDenied: '做不到的：翻看你的聊天记录、账号资料、文件',
      docsCta: '想自己核对？看完整的权限清单',
  ...
  supplyAdmin: {
    ...
    settlement: {
      ...
      shareRatio: '分成比例（默认 / Claude）',
      shareRatioHint: '基数是消费者实付金额（不是官方价）。0.7 表示供给者拿 70%。上限 {max}。',
      openaiShareRatio: 'OpenAI 供给分成（可选）',
      openaiShareRatioHint: '只作用于 OpenAI 供给账号。0 = 不单独配、回落上面的默认分成。两平台订阅经济账不同，可分别定。',
      freezeHours: '冻结小时数',
    ...
    pool: {
      ...
      openaiTitle: 'OpenAI 供给池（可选）',
      openaiHint: '上面三项是 Claude（anthropic）默认池；这一块是 OpenAI 平台的独立池。供给组填 0 = 不配 OpenAI 池。',
    ...
    probation: {
      ...
      advancedTucked:
        '工程参数（探测间隔、达标次数、排空窗、探测模型、闲置失败阈值）已从面板收起：默认值即推荐值，settings API 仍可手工调整。',
      clampNotice: '这一组参数越界时后端会夹回区间并保存（不会报错），保存后表单显示的是库里真正存下的值。',
      idleProbeEnabled: '开启闲置号复检',
      idleProbeEnabledHint:
        '入池之后，凭证被撤销和真实请求被拒都已经有人管；唯独「订阅降级或退订、但授权还有效」在账号上没有任何痕迹——一个从不接单的号会一直被当成在役。开启后，平台会对长期零流量的号定期探一次，连续拿到凭证失效/无额度才把它停下来。正在限流或过载的号不探：额度被打满是供货成功的证据。',
      idleAfterHours: '闲置判定（小时）',
      idleAfterHoursHint:
        '多久没接过单算闲置，同时也是两次复检的最小间隔。探测花的是供给者自己的订阅额度，所以这个数不宜小。范围 {min}–{max} 小时。',
      save: '保存观察期参数',
      saved: '观察期参数已保存'
    },
    ...
    balance: {
      title: '新会话产出均衡',
      description: '新会话优先落到今日产出最少的共享号上。补的是 LRU 补不上的那一块——LRU 均衡的是会话数，而共享者的收益取决于用量。默认关。',
      enabled: '启用产出均衡',
      enabledHint: '关闭时调度顺序与从前逐字一致（优先级 → 负载率 → LRU），且不发任何额外查询。粘性会话任何时候都不受影响。',
      bandUsd: '分档带宽（美元 / 官方牌价）',
      bandUsdHint: '今日产出相差不到这个数的号算同一档，档内仍按 LRU 打散。默认 {d}；填 0 会被夹回默认值——严格「选最小」会让并发的新会话一起涌向同一个号。',
      save: '保存均衡配置',
      saved: '均衡配置已保存'
    },
    incentive: {
      ...
      startAt: '起算日（UTC）',
      startAtHint:
        '这期活动的时间原点：在线天数按活动分桶，这一天之后才开始给这期计数，所以每期都从 0 开始，第二期不会把第一期攒下的天数算进来。**不能填过去的日期**——桶只能往前累加，历史补不回来，后端会直接拒绝。已经开跑的活动回来调档位时，这一栏保持原样即可。',
      newUsersOnly: '只发给新供给者',
      newUsersOnlyHint:
        '新 = 起算日之前名下没有任何供给账号，含已解绑的。老贡献者加挂新号也拿不到。含已解绑这一条是必须的：否则「先解绑再挂回来」就能把自己洗成新人。',
    ...
    homepageBanner: {
      ...
      ctaUrlZh: '按钮链接（中文访客）',
      ctaUrlEn: '按钮链接（英文访客）',
      ctaUrlHint: '只接受 http/https。两种语言各指各的落地页——文档站是双语的，中文访客点过去该看到中文页。只填一个也行，另一种语言会回退到已填的那个。填了按钮文字就必须至少填一个链接：有按钮点不动比没有按钮更像故障，后端会直接拒绝。',
      variant: '样式',
      variantPromo: '活动（醒目）',
      variantInfo: '通知（克制）',
