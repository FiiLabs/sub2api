export default {
  ...
  proof: {
    ...
    verify: {
      ...
      clockSkew: {
        title: '你的设备时间不准——这不是安全问题',
        note: '好消息：所有加密和硬件检查都通过了。唯一没通过的是“时效性(freshness)”，而这也是全部检查里唯一一项要读你设备时钟的——它对不上，是因为你这台电脑/手机的时间设置本身偏了，跟 ApexOne 的机密硬件、以及你 prompt 的安全性无关（那些都已验证通过）。',
        detail: '你的设备时间比标准时间{direction}约 {amount}，超出了我们允许的误差范围（±5 分钟），所以这一项没通过。',
        fix: '解决办法：在系统设置里打开“自动设置日期和时间”，或把时间校准到正确值，然后刷新本页重新验证即可。',
        ahead: '快了',
        behind: '慢了',
        minutes: '{n} 分',
        seconds: '{n} 秒'
  ...
  home: {
    ...
    landing: {
      hero: {
        ...
        subtitle:
          '通过一个运行在已认证 TEE 内的统一 API，访问真实的 Claude、GPT 和 Gemini——全精度、不量化、不静默替换。用密码学证明你付费购买的模型就是实际响应的模型，也证明没有人（包括我们）能读取你的提示词，同时价格仅为官方直连的五折。',
        viewDocs: '查看文档',
        stats: {
          uptime: '可用性 SLA',
          attested: 'TEE 认证路由',
