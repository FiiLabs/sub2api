...
export default {
  ...
  supplyAdmin: {
    ...
    homepageBanner: {
      ...
      ctaUrlZh: 'Button link (Chinese visitors)',
      ctaUrlEn: 'Button link (English visitors)',
      ctaUrlHint: 'http/https only. Each language points at its own landing page — the docs site is bilingual, so a Chinese visitor should land on the Chinese page. Filling in just one is fine; the other language falls back to it. A button label requires at least one link — a button that goes nowhere reads as a fault, so the backend rejects it outright.',
      variant: 'Style',
