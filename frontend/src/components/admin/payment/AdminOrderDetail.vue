    ...
    <div v-if="order" class="space-y-4">
      <div class="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <p class="text-sm font-medium text-gray-900 dark:text-white">{{ formatCurrencyAmount(baseAmount, order.currency, locale) }}</p>
        </div>
        <div v-if="order.fee_rate > 0">
          <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.orders.fee') }} ({{ order.fee_rate }}%)</p>
          <p class="text-sm font-medium text-gray-900 dark:text-white">{{ formatCurrencyAmount(feeAmount, order.currency, locale) }}</p>
        </div>
        <div>
          <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.orders.payAmount') }}</p>
          <p class="text-sm font-medium text-gray-900 dark:text-white">{{ formatCurrencyAmount(order.pay_amount, order.currency, locale) }}</p>
        </div>
        <div v-if="order.amount !== order.pay_amount">
          <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.orders.creditedAmount') }}</p>
          <p class="text-sm font-medium text-gray-900 dark:text-white">{{ formatCurrencyAmount(order.amount, creditedCurrency(order.order_type), locale) }}</p>
        </div>
        <div>
      ...
      >
        ...
        <div class="grid grid-cols-2 gap-2 text-sm">
          <div>
            ...
            <span class="ml-1 font-medium text-red-700 dark:text-red-300">{{ formatCurrencyAmount(order.refund_amount, creditedCurrency(order.order_type), locale) }}</span>
          </div>
...
import { statusBadgeClass, canRefund as canRefundStatus, formatOrderDateTime, formatCurrencyAmount, creditedCurrency } from '@/components/payment/orderUtils'
