...
// 刻意不引入 `v-html` 分支:落地页是本产品可验证性主张的门面,不值得为一个我们不用
// 的运营开关多开一个(哪怕是管理员门控的)XSS 面,iframe 分支还要放宽 CSP frame-src,
// 而 CSP 会被度量进镜像。
// 保留文件而非删除:① 下次同步 upstream 时不会变成 delete/modify 冲突;② 决策理由
// 留在原地,将来若决定采纳这两个开关,补齐 HomeView 的分支后把 .skip 去掉即可。
