...
> **状态补记（2026-08-26）：上面两段是首版切片当时的范围划分，原样保留；下面这张表才是此刻的真实状态。** 后续刀次吃掉了其中四项，「在范围」那段里的「人工打款」也已被推翻——现在读这一节，请以本表为准。
>
> | 原「出范围」项 | 现状 |
> |---|---|
> | 自动打款 | **已落地**，但走的不是支付通道而是链上（M2–M6，§9.5–§9.10）。「提现的打款动作是人工的」这句已作废，M6b 起提现是**链上-only**，人工渠道整条下掉 |
> | API key 型供给 | **已落地**（M7 中转接入，§9.11） |
> | 封禁率报表 | **已落地**（`SupplierIncidentService.Summary`） |
> | 平台熔断 | **未做**。现有的 `GuardOnboarding` 是**每人**接入闸，不是整池级熔断——两者别混 |
> | 新供给者保底 floor / 自动代理池 / 拟人化时段节流 / 订阅档位识别 / User 信誉分 / 连接级 draining | **未做**，代码里无痕迹 |
...
顺带钉死一处易错点：`AccountQuotaCost = TotalCost × AccountRateMultiplier` 是**账号维度记账口径（近似官方价）**，拿它当分成基数会让供给者按**官方价**拿分成，而消费者只按**打过折的倍率**付费——每笔都亏，且倍率压得越低亏得越多。已有单测 `TestSupplierSettlementBasisUsesConsumerActualPayment` 守住。
...
| 接入熔断 | `internal/service/supplier_incident_service.go`（`GuardOnboarding`）、`setting_supply_onboarding.go`（两个新字段） | — | 反复往平台塞坏号的人被挡在接入路径上。`supply_onboarding_settings` 加 `max_incidents_per_user`（**2026-08-27 起默认 3 = 开**，早先是 0）与 `incident_window_hours`（默认 7 天）。判据放在事件服务而不是接入服务：接入服务不需要知道事件是什么。**查询失败放行**，与另外两道闸相反，理由见 §3.10 |
...
**溢出不换价签，这是白捡的**。消费者价来自 `apiKey.Group.RateMultiplier`（`gateway_usage_billing.go:806-813`），而调度用的 group 是调度函数内部的局部变量，从不回传给计费。所以溢出只换供货来源：消费者按自己买的那一档付钱，平台自吃「按自营成本供货却按供给池价收费」的差额。**这笔差额的大小随倍率变化**——倍率压得越低，每次溢出亏得越多，所以调价时必须连带重估溢出敞口（自营池装订阅号还是按量 API Key，结论完全相反）。这既是对的（供给干涸是平台的问题，不该让消费者按原价结账），也不是新造的语义——已有的 claude-code 降级早就在跑同一条路：账号来自 fallback 分组、计价来自 apiKey 分组。
...
**渠道不是配出来的，是金库能力派生的**（M6b 改，§9.10）。~~早先这里是一张运营手填的白名单，配套一条"按完全相等匹配、改名等价于下线"的规矩。~~ 链上-only 之后那条规矩连同白名单一起作废：`settleableChannels()` 遍历代码里的链上渠道注册表，逐个问链上客户端"这条链上的这种币，你结算得了吗"，答得上来的才是渠道。**唯一事实只能有一个**——白名单里配着 `BSC-USDT`、金库却换成了 USDC 的部署，会给供给者一个必定失败的选项。`supply_withdrawal_settings.channels` 字段在 struct 上还留着（兼容旧 JSON、保存时结转现值），但**代码不读它**，管理端的编辑入口也已撤掉。

**「开着但金库没配好」必须能被两边分别看见**。这是这一组最容易出现的坏状态：开关打开了、金库结算不了任何币种，于是每一次申请都被硬拒。供给者侧因此把"平台没开提现"和"开了但渠道在维护"画成两段不同的文案——后者明说是平台侧配置问题、不要重试；管理端那张卡片在这个状态下把 banner 画成红色。`options` 接口的 `available` 是**两个条件的与**（开着 且 `settleableChannels()` 非空），前端不自己拼这个判断。撤白名单那一轮踩过一次坑：编辑入口撤了、`validate()` 里"开着就必须给渠道"那条却留着，于是界面发一个空列表过去、保存被自己的校验锁死（§9.10 补记二）——**撤一个入口时要把它的校验一起送走**。
...
**熔断闸默认开着（2026-08-27 起，7 天 3 次），且查询失败一律放行。** 打开它是**放开每人账号数上限的对价**：数量与风险无关，失效率才有关——前两道闸数的是「你有几个号」，这道数的是「你的号坏了几个」。但它有一个真实的误伤场景：上游一次大面积封号会在几分钟内给一大群正常供给者各记一条事件，这道闸会紧接着把他们全部挡在门外，症状是「平台在最需要供给的那一刻拒绝了所有供给」——真撞上了，临时调回 0 是正确的应急动作。 后半句是与另外两道接入闸（每人 / 每 IP）**相反**的方向，理由不对称：那两道数的是当下的账号数，一次简单 COUNT，失败几乎只意味着库挂了，而库挂了接入本来也走不通；这道数的是历史事件，它失败时接入路径其余部分完全正常——为一次统计查询的抖动把一个正常供给者挡在门外，代价大于放进来一个坏号（下一轮扫描照样记下他，闸恢复后照样拦得住）。数的是**事件次数**而不是坏号个数：同一个号反复坏是最强的信号，按号去重会把它压成 1。窗口与阈值分成两个配置字段（`max_incidents_per_user` / `incident_window_hours`），且窗口的 `0` 与本仓其它所有上限的 `0` 含义**不同**——那些是"不限"，这个只是"没填"，兜默认 7 天，因为 0 小时的窗口会让一道看起来开着的闸永远拦不住任何人。给用户的错误文案刻意不写具体阈值（那等于把绕过方法一起发给他），也刻意说"最近"和"稍后再试"（被上游一次大面积封号波及的正常供给者需要知道这不是永久的）。
...
第二个 key `supply_pool_settings`（`enabled` / `supply_group_id` / `overflow_group_id` / `daily_overflow_limit`）同样默认关，装的是路由而不是钱。第三个 key `supply_probation_settings`（见 §3.5）装的是准入与下线时序，`enabled` 同样默认关。第四个 key `supply_agreement_settings` 装协议文本与版本号，版本留空 = 自助接入整条关着（见 §3.3）。第五个 key `supply_withdrawal_settings`（见 §3.7）装提现开关、起提额、每人未决单上限、说明文本与运营通知收件人，`enabled` 同样默认关。~~收款渠道白名单~~ 在 M6b 之后不再是配置项——渠道由金库能力派生（§3.7 第五条），`channels` 字段仅为兼容旧 JSON 保留、代码不读。第六个 key `supply_onboarding_settings`（见 §3.3）装每人 / 每来源 IP 的账号数上限，两个字段都以 0 = 不限为约定，每 IP 那道默认就是 0。**刻意分成六个 key**：这几组配置因完全不同的原因变动（动分成比例 / 动兜底池 / 动准入门槛 / 改协议 / 换收款渠道 / 收紧刷号），合成一个会让几次意图完全不同的修改共用一条审计记录。

六个 key 都已有管理端 API 与界面（`/admin/supply-market` 页，六张卡各自一个保存按钮；M6 起多出第七个 key `supply_payout_chain_settings` 装金库配置，见 §9.10）。**四个总开关默认全关，这是上线策略而不是待办**：代码随版本先进生产，管理员逐个打开。但「提现开着 + 金库结算不了任何币种」是一个必须避开的中间态，它在两边的表现见 §3.7。

> **⚠️ 下表是「代码默认值」，不是任何一个部署实际在跑的值。**
>
> 这两件事必须分清，否则会出一类很难查的事故：本地/预发按运营决策调过的参数
> **只活在那一个数据库里**，重新部署或换环境时全部回落到下表的默认值——而症状
> 是「代码一行没改，分成比例和冻结期却变了」。
>
> 代码默认值刻意站在**保守**的那一侧（开关全关、分成偏低、冻结偏长、熔断关闭），
> 因为这是开源项目要发给任何人的初值；**具体部署该用什么值是运营决策**，记在
> 各自的运维文档里，不进这个仓库。
>
> **换环境/重新部署时，这七个 settings key 加上供给池/自营池两个分组的
> `rate_multiplier` 必须逐项对照运维文档重新配一遍**——它们都不随代码走。

| 参数 | 代码默认值 | 说明 |
|---|---|---|
| `enabled` | `false` | 总开关。默认关就是上线策略：代码先随版本进生产、在计费主链路上待着，管理员显式打开才开始动钱 |
| `share_ratio` | **0.80** | 供给者分成，基数 = 消费者实付（**非官方价**）。它与消费者侧倍率联动，两者一起定才有意义：比例高不等于供给者拿得多，倍率压得越低、同样的比例换来的钱越少 |
| `spend_from_wallet_first` | `false` | 与 `enabled` 分开是有意的：可以先只开入账让供给者攒着，等钱包侧观察稳了再打开消费出口 |
| 供给池 `rate_multiplier` | （分组表默认 `1.0`） | 消费者价 = 该倍率 × 官方价。**不在这两个 key 里**，是分组自己的 `rate_multiplier` 字段，管理端分组页配。定价是运营决策，不该有代码默认值——留 1.0（官方原价）等于「没配就不打折」，是安全的那一侧 |
| `freeze_hours` | **`72`（3 天）** | 须 ≥ 支付通道拒付窗，否则不变量 2「已释放 = 拒付安全」不成立。代码侧只 clamp 到 90 天上限（`SupplierFreezeHoursMax = 2160`），**拦不住「配小了」**，所以 `setting_supplier_test.go` 里钉死了这个字面量当绊线：改它的人必须先撞一次那条断言、并把答案写下来。**2026-08-27 从 720（30 天）改到 72**：冻结期对供给者是纯粹的等待成本，而供给是这个市场的瓶颈侧，30 天的等待在招募时是硬伤；3 天足以覆盖盗卡这类明显欺诈（几天内就会被持卡人发现），那是拒付里金额占比最大的一类。**代价**：3–30 天之间发生的拒付追不回来，平台自吃，损失落在 `payment_disputes.uncovered_basis` 上——**上线一个月后按那一列复核**，超出预期就调回去或把比例准备金做出来（释放大部分、留一小部分更久，还没做）。另有一条能从根上绕开这个权衡的路：加密货币充值产生的消费根本不需要冻结（链上支付不可拒付），能追溯资金来源的话那部分可即时释放 |
| 自营池 `rate_multiplier` | （分组表默认 `1.0`） | 与供给池同理。注意**溢出不换价签**（§9.6 / `gateway_usage_billing.go`）：溢出流量按**消费者密钥所属分组**计价，所以这个字段只对「直接绑定到自营池的密钥」生效，对溢出流量不起作用 |
...
| 每人账号数上限 | `5`（**兜底值，非策略值**） | 落在 `supply_onboarding_settings.max_accounts_per_user`，0 = 不限。**这一项刻意不跟随运营策略**：运营侧此刻配的是 0（放开——数量与风险无关，挡不住刷号的人却把成规模供货的专业供给者关在门外），但代码默认值留在 5，因为它同时是**故障路径的兜底**（配置读不出来时退回这里，那一刻「不限」是错的答案）。把它改成 0 去跟随策略，会顺带把故障路径从 fail-safe 变成 fail-open 且没有任何症状。见 §3.3 |
...
> **本节两处已被后续推翻，读之前先知道（2026-08-26）**：① 手续费机制整体拆除，本节关于「从 `amount` 内切」「`SanitizeChainFee` 收敛到 8 位」「估不出报 503」「报价不是承诺」的全部推理**已作废**，新单 `fee_amount` 恒 0、gas 由金库承担——见 §9.13；② 「结算不了就退回人工工单」这条决定在 M6b 被反转成**拒绝建单**，理由是人工队列已整条下掉——见 §9.10。以下正文按当时的判断原样保留。
