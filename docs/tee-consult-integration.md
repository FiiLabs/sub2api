...
## 本分支（tee-control）的差异：纯控制面

本分支采用「纯控制面（全部走 TEE）」，**不区分 personal/team 模式、不依赖 billing_subject**：

- consult `/pre` **接受所有 key**（移除原 `IsTeamMode()` gate）。
- 用量经占位 account 走 sub2api **原始 user 维度计费**（无 `billing_subject`/`QuotaSubjectScoped`）。
- **数据面（决策①D：无条件 410 桩，写死非 flag）**：`registerRoutes` 用 `RegisterDataPlaneDisabled(r)` 取代 `RegisterGatewayRoutes(...)`，对 `/v1`、`/v1beta`、`/responses`、`/backend-api/codex` 一律返回 410 `DATA_PLANE_DISABLED`。`RegisterGatewayRoutes` 与转发 handler 保留为无路由可达的死代码，以开源 + 路由审计自证「sub2api 不转发任何第三方，用户请求只在 TEE 发生」。
- 因此原文中「team key / personal key 互斥」「IsTeamMode 判定」「双模式」的描述在本分支不再适用——数据面对所有请求统一 410。
